...
function maybeGetHackableProxySync(connection, busName, objectPath) {
    try {
        ...
        log(`${busName}:${objectPath} does not export the Hackable interface, using default toolbox: ${String(e)}`);
        return null;
    }
}

function createHackToolboxSkeletonOnPath(connection, objectPath, targetBusName, targetObjectPath) {
    log(`Creating toolbox for ${targetBusName}:${targetObjectPath}`);
    let skeleton = new HackToolbox.ToolboxSkeleton({
...
const HackToolboxMainWindow = GObject.registerClass({
    ...
    _init(params) {
        ...
        let connection = this.application.get_dbus_connection();
        this.hack_toolbox_skeleton = createHackToolboxSkeletonOnPath(connection,
                                                                     GLib.build_filenamev([
                                                                         this.application.get_dbus_object_path(),
                                                                         'window',
                                                                         String(this.get_id())
                                                                     ]),
                                                                     this.target_bus_name,
                                                                     this.target_object_path);
...
const HackToolboxApplication = GObject.registerClass(class extends Gtk.Application {
    _init() {
        ...
        let showForDBusObject = new Gio.SimpleAction({
            name: 'show-for-dbus-object',
            parameter_type: new GLib.VariantType('(ss)')
        });
        showForDBusObject.connect('activate', (action, parameterVariant) => {
            let [busName, objectPath] = parameterVariant.deep_unpack();
            log(`Call showForDBusObject with ${JSON.stringify(parameterVariant.deep_unpack())}`);
